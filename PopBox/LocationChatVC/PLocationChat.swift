...
//  Popbox
//
//  Created by Chandan  on 11/24/17.
...
class PLocationChat: UIViewController{
    ...
    var chatUsers  :[PPeople] = [PPeople]()
    var currentDay : String = ""
    
    //var manager = SocketManager(socketURL: URL(string: "http://52.205.90.191/chat_module/chat//")!, config: [.log(true), .forcePolling(true)])
//    http://notosolutions.net:3000/
    
    let socket: SocketIOClient = SocketIOClient(socketURL: NSURL(string: "http://52.205.90.191:3000/")! as URL)
...
//        self.locationInfo.place_id = "4db1a2035da32cf2df60580b"

        //self.connectionEstablish()
//         self.getChatHistory(index: 0, purpose: "")
        getLocationDetails(placeId: self.locationInfo.place_id) {
...
//        -- msg used when we get response on reply
//        -- send is when we get simple response
...
//
    ...
    func muteUnmute(title : String)
    {
        ...
        if title == "Mute"
        {
            ...
            self.showToastAlert(message: "Notifications muted")
        }
        else
        {
            self.tempTitle = "Mute"
            UserDefaults.standard.set(false, forKey: "isMute")
            self.showToastAlert(message: "Notifications unmuted")
        }
//
                ...
                if let responseDict = response as? Dictionary<String, AnyObject> {
                    if let successFull = responseDict[pSuccess] as? Bool {
                        if (successFull) {
                            ...
                            self.user_id = kAppDelegate.objUserInfo.id
                            ...
                            if let userList = data[pUsers] as? NSArray
                            {
                                
                                for dict in userList
                                {
                                    let p = PPeople().initPeopleProfileData(infoDict: dict as! Dictionary<String, AnyObject>)
//                                    if p.status != .inactive {
                                        self.locationInfo.users.append(p)
                                        self.friendList.append(p)
//                                    }
                                }
                            }
                            
                            let friends = self.locationInfo.users.filter({ (people) -> Bool in
                                if people.is_friend == 1 && people.visible != .invisible{
                                    return true
                                }
                                return false
                            })
                            let hiddenFriends = self.locationInfo.users.filter({ (people) -> Bool in
                                if people.is_friend == 1 && people.visible == .invisible{
                                    return true
                                }
                                return false
                            })
                            let others = self.locationInfo.users.filter({ (people) -> Bool in
                                if people.is_friend == 0 {
                                    return true
                                }
                                return false
                            })
                            ...
                            if friends.count != 0 {
                                if friends.count > 1 {
                                    let sortedList = friends.sorted { $0.f_name < $1.f_name }
                                    
                                    let str = sortedList[0].f_name.capitalized + ", " + sortedList[1].f_name.capitalized
                                    if others.count+hiddenFriends.count > 0 {
                                        self.lbl_peoples.text = str + ", You, " + String(others.count+hiddenFriends.count) + " others"
                                    }else{
                                        self.lbl_peoples.text = str + ", You"
                                    }
                                }else{
                                    let str = friends[0].f_name.capitalized
                                    if others.count+hiddenFriends.count == 1 {
                                        self.lbl_peoples.text = str + ", You, " + String(others.count+hiddenFriends.count) + " other"
                                    }else if others.count+hiddenFriends.count > 1 {
                                        self.lbl_peoples.text = str + ", You, " + String(others.count+hiddenFriends.count) + " others"
                                    }else{
                                        self.lbl_peoples.text = str + ", You"
                                    }
                                ...
                                let c = others.count+hiddenFriends.count
                                if c == 1 {
                                    self.lbl_peoples.text = "You, " + String(others.count+hiddenFriends.count) + " other"
                                }else if others.count+hiddenFriends.count > 1 {
                                    self.lbl_peoples.text = "You, " + String(others.count+hiddenFriends.count) + " others"
                                }else{
                                    self.lbl_peoples.text = "You"
                                }
                            }
                            //                            let p = PPeople()
...
//            MBProgressHUD.showAdded(to: self.view, animated: true)
        ...
        let param = [pBox_id:self.locationInfo.place_id,pUser_id:userID]
        debugPrint(param)
        Alamofire.request(kApiChatHistory, method: .post, parameters: param).responseJSON { response in
            DispatchQueue.main.async {
                MBProgressHUD.hideAllHUDs(for: self.view, animated: true)
            }
            if let jsonObject = response.result.value as? [String: AnyObject]{
                print(jsonObject)
                if let status  = jsonObject[pSuccess] as? Bool
                {
                    self.msgArry.removeAll()
                    self.dateTemp = self.tempDateFormattr.string(from: AppUtility.getDate())
                    if(status == true)
                    {
                        if let response = jsonObject[pData] as? NSDictionary
                        {
                            if let users = response[pUsers] as? [NSDictionary]
                            {
                                for dict in users
                                {
                                self.chatUsers.append(PPeople().initPeopleProfileData(infoDict: dict as! Dictionary<String, AnyObject>))
                                }
                            }
                            if self.chatUsers.contains(where: { (p) -> Bool in
                                if p.id == self.userID {
                                    return true
                                }
                                return false
                            }) == false{
                                self.joinChat(user: self.userID)
                            }
                            if let results = response["results"] as? [NSDictionary]
                            {
                                for dict in results
                                {
                                    let userid = "\(dict[pUser_id]!)"
                                    if userid != ""
                                    {
                                ...
                                if(self.msgArry.count > 0)
                                {
                                    ...
                                    else if(purpose == MediaType.Image.rawValue)
                                    {
                                        self.view.layoutSubviews()
...
//            MBProgressHUD.showAdded(to: self.view, animated: true)
        ...
        let param = [pPlace_id:self.locationInfo.place_id,pUser_id:user]
        debugPrint(param)
 
        Alamofire.request(kApiJoinChat, method: .post, parameters: param).responseJSON { response in
            DispatchQueue.main.async {
                MBProgressHUD.hide(for: self.view, animated: true)
            }
            if let jsonObject = response.result.value as? [String: AnyObject]{
                ...
                if let status  = jsonObject[pSuccess] as? Bool
                {
                    if(status == true)
                    {
                        ...
                        let p = PPeople()
                        p.id = self.userID
                        p.f_name = kAppDelegate.objUserInfo.firstName
                        p.l_name = kAppDelegate.objUserInfo.lastName
                        p.profile_pic_thumb  = kAppDelegate.objUserInfo.profileThumb
                        self.chatUsers.append(p)
                        
                    }else{
                         NSLog("join API>false")
                    }
                }
...
//        let time : String = datfomattr.string(from: Date())
        let time : String =  AppUtility.getDate().getUTCDateTimeString()
        ...
        param = ["message_id":chat.id!,
                 ...
                 "is_like": like_msg,
                 "user_id":userID,
                 "box_id":self.locationInfo.place_id,
    ...
    func addAttachment(attachmentName  :String)
    {
        ...
        if(self.selectedImage != nil && attachmentName == MediaType.Image.rawValue)
        {
        ...
        if(self.selectedVideoURL != nil && attachmentName == MediaType.Video.rawValue)
        {
            do
            {
                                ...
                                case .success(let upload, _, _):
                                    upload.responseJSON { response in
                                        ...
                                        do
                                        {
                                            ...
                                            if let response = jsondata[pData] as? NSDictionary
                                            {
                                                ...
                                                //let imgName
                                                if(chatuser.attachment_name?.lowercased() == MediaType.Image.rawValue)
                                                {
                                                ...
                                                if(chatuser.attachment_name?.lowercased() == MediaType.Video.rawValue)
                                                {
        ...
        Alamofire.request(urlStr, method: .post, parameters: params ).responseJSON { response in
            ...
            if let jsonObject = response.result.value as? [String: AnyObject]{
                ...
                if let status  = jsonObject["status"] as? Bool
                {
                    ...
                    {
                        self.selectedIndex = -1
                        self.showToastAlert(message: "Message reported")
...
//            let time : String = datfomattr.string(from: Date())
            ...
            if(selectedIndex == -1)
            {
                ...
                param = ["user_id":userID, "box_id":self.locationInfo.place_id, "parent_id":"0", "message":msgText,"is_flagged":"1","status":"1","created_at":time,"type":"",
            ...
            if(selectedIndex < msgArry.count && self.selectedIndex > 0)
            {
                ...
                let replyUSer : [String : Any] = ["id": replyData.id ?? "",
                                                  ...
                                                  MediaType.Video.rawValue: replyData.video ?? "",
...
//                    
    ...
    @objc func showProfile(_ sender: UITapGestureRecognizer) {
        ...
        if v.tag != 0 {
            ...
            let f = PFriendsInfo()
            f.id = self.chatUsers[v.tag].id
            f.full_name = self.chatUsers[v.tag].full_name
            f.profile_pic_thumb = self.chatUsers[v.tag].profile_pic_thumb
            f.profile_pic = self.chatUsers[v.tag].profile_pic
            f.status = self.chatUsers[v.tag].status
            f.is_friend = self.chatUsers[v.tag].is_friend
            vc.friend = f
    ...
    @objc func longPressGesture(sender :UILongPressGestureRecognizer)
    {
        ...
        if(msgActionContainer.isHidden == true)
        {
            ...
            if(selectedIndex < msgArry.count && self.selectedIndex > 0)
            {
                ...
                {
                    if(chatObj.attachment_name?.lowercased() == MediaType.Image.rawValue || chatObj.attachment_name?.lowercased() == MediaType.Video.rawValue)
                    {
    ...
    func registerCellClass()
    {
        ...
        chatTablView.register(UINib.init(nibName: "DateViewCell", bundle: nil), forCellReuseIdentifier: "datebubble")
        //--------------------------My Chat Text--------------------------
        chatTablView.register(UINib.init(nibName: "ChatByMeCell", bundle: nil), forCellReuseIdentifier: "chatbymeCell")
        //--------------------------Other User Chat Text------------------
        chatTablView.register(UINib.init(nibName: "ChatByOtherCell", bundle: nil), forCellReuseIdentifier: "chatbyotherCell")
        //------------------------- Reply By Other User Chat--------------
        chatTablView.register(UINib.init(nibName: "ReplyViewCell", bundle: nil), forCellReuseIdentifier: "replyotherCell")
        //--------------------------My Reply Chat-------------------------
        chatTablView.register(UINib.init(nibName: "MyMsgReplyCell", bundle: nil), forCellReuseIdentifier: "myReplyCell")
        //------------------------- My Image Chat-------------------------
        chatTablView.register(UINib.init(nibName: "ChatImageByMeCell", bundle: nil), forCellReuseIdentifier: "myImageCell")
        //------------------------- Other User Image Chat-----------------
        chatTablView.register(UINib.init(nibName: "ChatImageByOtherCell", bundle: nil), forCellReuseIdentifier: "otherImageCell")
        
        //------------------------- My Video Chat-------------------------
        chatTablView.register(UINib.init(nibName: "MyChatVideoCell", bundle: nil), forCellReuseIdentifier: "myChatVideo")
        //------------------------- Other user Chat Video-----------------
        chatTablView.register(UINib.init(nibName: "OtherChatVideoCell", bundle: nil), forCellReuseIdentifier: "otherChatVideo")
        //------------------------- Other User Video Reply----------------
        chatTablView.register(UINib.init(nibName: "OtherVideoReplyCell", bundle: nil), forCellReuseIdentifier: "othervideoReply")
        
        //------------------------- My Video Reply Chat-------------------
        chatTablView.register(UINib.init(nibName: "MyVideoReplyCell", bundle: nil), forCellReuseIdentifier: "myvideoReply")
        
        //------------------------- My Image Reply Chat--------------------
        chatTablView.register(UINib.init(nibName: "ReplyImageCell", bundle: nil), forCellReuseIdentifier: "myReplyImage")
        
        //------------------------- Other User Image Reply Chat------------
        chatTablView.register(UINib.init(nibName: "ReplyChatOtherCell", bundle: nil), forCellReuseIdentifier: "replyOtherImage")
        //------------------------- My Reported Msg------------------------
        chatTablView.register(UINib.init(nibName: "MyReportedMsgCell", bundle: nil), forCellReuseIdentifier: "myMsgReportCell")
        //------------------------- Other User Msg reported----------------
        chatTablView.register(UINib.init(nibName: "OtherReportedMsgCell", bundle: nil), forCellReuseIdentifier: "otherMsgReportCell")
        //------------------------- Report Option--------------------------
...
//        let attrString = NSAttributedString.init(string: text, attributes: [NSAttributedStringKey.font:font])
    ...
    func customizeDropDown(_ sender: AnyObject) {
        ...
        dropDowns.forEach {
            ...
            $0.customCellConfiguration = { (index: Index, item: String, cell: DropDownCell) -> Void in
                ...
                if self.locationInfo.ambassador_status == -1 {
                if index == self.menuOptions.count - 1 {
                    cell.optionLabel.textColor = hexStringToUIColor(cDarkGrey)
                }else{
                    cell.optionLabel.textColor = hexStringToUIColor(cDarkGrey)
                    }
                }else{
                    if index == self.menuOptions.count - 1 {
                        cell.optionLabel.textColor = hexStringToUIColor(cLightGreyNew)
                    }else{
                        cell.optionLabel.textColor = hexStringToUIColor(cDarkGrey)
                    }
                }
    ...
    func setupChooseArticleDropDown(chooseButton:UIButton) {
        ...
        dropDown.selectionAction = { [unowned self] (index, item) in
            ...
            self.menuOptionsEvent(index: index)
        }
    }
    ...
    @IBAction func backAction(_ sender: Any) {
        ...
        kAppDelegate.updateViewLocation(placeId: self.locationInfo.place_id, isViewing: 0) { (status) in
    ...
    @IBAction func favouriteAction(_ sender: Any) {
        ...
        if  self.locationInfo.isFav == 1 {
            is_fav = 0
            self.btn_fav.setImage(#imageLiteral(resourceName: "star_white_iPhone"), for: .normal)
        }else{
            self.btn_fav.setImage(#imageLiteral(resourceName: "star_iPhone"), for: .normal)
        }
        self.locationInfo.isFav = is_fav
        kAppDelegate.addFavouriteRequest(user_id: self.user_id, place_id: self.locationInfo.place_id, is_fav: is_fav) { (status,isFav) in
            DispatchQueue.main.async {
                MBProgressHUD.hide(for: self.view, animated: true)
            }
            if status {
                ...
                self.locationInfo.user_count = String(self.locationInfo.users.count + 1)
...
//        joinChat(user: userID)
//        let ambassador = LocationChatStoryboard.instantiateViewController(withIdentifier: "PAmbassadorVC") as! PAmbassadorVC
//        ambassador.locationInfo = self.locationInfo
//        self.navController.pushViewController(ambassador, animated: false)

        let actoinSheet = UIAlertController.init(title: nil, message: nil, preferredStyle: .actionSheet)
         actoinSheet.addAction(UIAlertAction.init(title: "Location info", style:  .default, handler: { (action) in
            self.menuOptionsEvent(index: Location.Profile.rawValue)
        }))
        
        actoinSheet.addAction(UIAlertAction.init(title: "Invite friends", style:  .default, handler: { (action) in
            self.menuOptionsEvent(index: Location.InviteFriends.rawValue)
        }))
        actoinSheet.addAction(UIAlertAction.init(title: "Be an Ambassador", style:  .default, handler: { (action) in
            
            self.menuOptionsEvent(index: Location.Be_Ambassador.rawValue)
        }))
        actoinSheet.addAction(UIAlertAction.init(title: "Cancel", style:  .cancel, handler: { (action) in
...
//        setupChooseArticleDropDown(chooseButton: (sender as? UIButton)!)
//        dropDown.show()
    }
    func menuOptionsEvent(index: Int){
        switch index {
        case Location.Profile.rawValue:
            self.showLocationProfileAction(self.btn_fav)
            
            //            case Location.Mute.rawValue:
            //
        //                break
        case Location.InviteFriends.rawValue:
            let vc = LocationChatStoryboard.instantiateViewController(withIdentifier: "PInviteMembersVC") as! PInviteMembersVC
            vc.locationInfo = self.locationInfo
            self.navController.pushViewController(vc, animated: false)
            
        case Location.Be_Ambassador.rawValue:
            //                let ambassador = LocationChatStoryboard.instantiateViewController(withIdentifier: "PAmbassadorVC") as! PAmbassadorVC
            //                ambassador.locationInfo = self.locationInfo
            //                self.navController.pushViewController(ambassador, animated: false)
            self.perform(#selector(ambassadorFunctionality), with: nil, afterDelay: 0.2)
        default: break
            
        }
    }
    @objc func ambassadorFunctionality(){
        if self.locationInfo.ambassador_status == -1 {
            let ambassador = LocationChatStoryboard.instantiateViewController(withIdentifier: "PAmbassadorVC") as! PAmbassadorVC
            ambassador.locationInfo = self.locationInfo
            self.navController.pushViewController(ambassador, animated: false)
        }
        else  if self.locationInfo.ambassador_status == kOFF {
            _ =  AlertController.alert(title: "Popbox Ambassador", message: "Your application to be Ambassador is in progress. We will keep you posted. Meanwhile, enjoy your moments on Popbox!")
        } else  if self.locationInfo.ambassador_status == kON {
            _ =  AlertController.alert(title: "Hi Popbox Ambassador", message: "You are already an Ambassador! Thanks for being part of our awesome team.")
        }
    }
...
//            self.updateLocationInfo()
//        }
//        if self.lbl_peoples.text == "Tap here for location info" {
//            return
//        }
        locationProfileVC = LocationChatStoryboard.instantiateViewController(withIdentifier: "PLocationProfileVC") as! PLocationProfileVC
        locationProfileVC.locationInfo = self.locationInfo
        locationProfileVC.locationDelegate = self
        locationProfileVC.navController = self.navController
        self.navController.pushViewController(locationProfileVC, animated: false)
    }
    
    func updateLocationInfo(){
        if locationProfileVC != nil {
            locationProfileVC.refreshDetails()
        }
        if self.locationInfo.isFav == 0 {
            btn_fav.setImage(#imageLiteral(resourceName: "star_white_iPhone"), for: .normal)
        }else{
            btn_fav.setImage(#imageLiteral(resourceName: "star_iPhone"), for: .normal)
        }
        
        place_photo = changeImageUrl(url: self.locationInfo.icon)
        
        if place_photo != "" {
            let url = URL(string: place_photo)
        ...
        lbl_location.text = self.locationInfo.name
    }

    
/*  Location Info API 
     New field in response :
     is_fav : 1 if place is marked as fav else 0
     friend : array (who are in same place and also added in friend list)
     not_friend : count(who are in same place but not friend)
     users_count : count(how many users are in same place)
    ...
    func getLocationDetails(placeId : String, completion:(() -> Void)?) {
        ...
        ServiceHelper.callAPIWithParameters(paramDict, method: .get, apiName: kApiPlaceInfo) { (response, error) in
            DispatchQueue.main.async {
                MBProgressHUD.hide(for: self.view, animated: true)
            }
                ...
                if let responseDict = response as? Dictionary<String, AnyObject> {
                    if let successFull = responseDict[pSuccess] as? Bool {
                        if (successFull) {
                            ...
                            let locationchatVC = LocationChatStoryboard.instantiateViewController(withIdentifier: "PLocationChat") as! PLocationChat
                            self.user_id = kAppDelegate.objUserInfo.id
                            self.locationInfo.place_id = placeId
                            self.locationInfo.name = "\(data[pName]!)"
                            self.locationInfo.photo = "\(data[pPhoto]!)"
                            self.locationInfo.rating = "\(data[pRating]!)"
                            self.locationInfo.phone_no = "\(data[pPhone_no]!)"
//                            self.locationInfo.latitude = "\(data[pLatitude]!)"
//                            self.locationInfo.longitude = "\(data[pLongitude]!)"
                            self.locationInfo.isFav = data[pIs_fav] as! Int
...
//Updating Location Chat screen when Item removed or added to Fav.
extension PLocationChat:LocationProfileDelegate {
...
extension PLocationChat : UITextFieldDelegate
{
...
extension PLocationChat: UITextViewDelegate
{
...
extension PLocationChat : UITableViewDelegate , UITableViewDataSource
{
        ...
        if(tableView == chatTablView)
        {
            if(indexPath.row < msgArry.count)
            {
                ...
                if(chatuser.id == "" || chatuser.id == nil)
                {
                    ...
                    let cell : DateViewCell = tableView.dequeueReusableCell(withIdentifier: "datebubble", for: indexPath) as! DateViewCell
                ...
                else
                {
                    if(chatuser.user_id == userID)
                    {
                        ...
                        if(chatuser.color == 1)
                        {
                            let myCell : MyReportedMsgCell = tableView.dequeueReusableCell(withIdentifier: "myMsgReportCell", for: indexPath) as! MyReportedMsgCell
                            ...
                            if(chatuser.attachment_name == "")
                            {
                                if(parentID > 0 && chatuser.reply != nil)
                                {
                                    if(chatuser.reply != nil)
                                    {
                                        ...
                                        {
                                            let myReplyCell : MyMsgReplyCell = tableView.dequeueReusableCell(withIdentifier: "myReplyCell", for: indexPath) as! MyMsgReplyCell
                                            ...
                                            {
                                                myReplyCell.otherUSer.text = replyChat.username!
//                                                myReplyCell.otherUSer.attributedText = friendNameWithDegree(String(format:"%@",replyChat.username!), "")
                                            }
                                        ...
                                        {
                                            if(replyChat.attachment_name?.lowercased() == MediaType.Video.rawValue)
                                            {
                                                let myvideoReply : MyVideoReplyCell = tableView.dequeueReusableCell(withIdentifier: "myvideoReply", for: indexPath) as! MyVideoReplyCell
                                                ...
                                                {
                                                    myvideoReply.otherUSer.text = replyChat.username!
//                                                    myvideoReply.otherUSer.attributedText = friendNameWithDegree(String(format:"%@",replyChat.username!), "")
                                                }
...
//                                                    myvideoReply.playerView?.playerLayer.videoGravity = AVLayerVideoGravity.resizeAspectFill
                                            ...
                                            if(replyChat.attachment_name?.lowercased() == MediaType.Image.rawValue)
                                            {
                                                let myReplyImage : ReplyImageCell = tableView.dequeueReusableCell(withIdentifier: "myReplyImage", for: indexPath) as! ReplyImageCell
                                                ...
                                                {
                                                    myReplyImage.otherUSer.text = replyChat.username!
//                                                    myReplyImage.otherUSer.attributedText = friendNameWithDegree(String(format:"%@",replyChat.username!), "")
                                                }
                                ...
                                {
                                    let likeCount : Int = chatuser.count!
                                    let myCell : ChatByMeCell = tableView.dequeueReusableCell(withIdentifier: "chatbymeCell", for: indexPath) as! ChatByMeCell
                                    let date = dateFormaatr.date(from: chatuser.created_at!)
                                    myCell.chattime.text = String(format:"%@",timeFormattr.string(from: date!))
                                    if decode(chatuser.message!) != nil
                                    {
                            ...
                            else
                            {
                                if(chatuser.attachment_name?.lowercased() == MediaType.Image.rawValue)
                                {
                                    let myImageCell : ChatImageByMeCell = tableView.dequeueReusableCell(withIdentifier: "myImageCell", for: indexPath) as! ChatImageByMeCell
                                    let date = dateFormaatr.date(from: chatuser.created_at!)
                                    let likeCount : Int = chatuser.count!
                                ...
                                if(chatuser.attachment_name?.lowercased() == MediaType.Video.rawValue)
                                {
                                    let myVideoCell : MyChatVideoCell = tableView.dequeueReusableCell(withIdentifier: "myChatVideo", for: indexPath) as! MyChatVideoCell
                                    let date = dateFormaatr.date(from: chatuser.created_at!)
...
//                        is_like = chatuser.likeResult.contains(where: { (like) -> Bool in
//                            if like.user_id == self.userID {
//                                return true
//                            }
//                            return false
//                        })
                        ...
                        if(chatuser.color == 1)
                        {
                            let otherCell : OtherReportedMsgCell = tableView.dequeueReusableCell(withIdentifier: "otherMsgReportCell", for: indexPath) as! OtherReportedMsgCell
                            let date = dateFormaatr.date(from: chatuser.created_at!)
                            otherCell.chatTime.text = String(format:"%@",timeFormattr.string(from: date!))
                            otherCell.userName.text = chatuser.username!
//                            otherCell.userName.attributedText = friendNameWithDegree(String(format:"%@",chatuser.username!), "")
                        ...
                        else
                        {
                            ...
                            var friendObj = PPeople()  //= FriendInfo([String : Any])
                            
                            let temp = self.chatUsers.filter({ (obj : PPeople) -> Bool in
                                return obj.id == chatuser.id!
                            })
                            
                            if(temp.count > 0)
                            {
                                friendObj = temp.first!
                            }else{
                                friendObj.id = chatuser.user_id!
                            }
                            ...
                            if(chatuser.attachment_name == "")
                            {
                                if(parentID > 0 && chatuser.reply != nil)
                                {
                                    if(chatuser.reply != nil)
                                    {
                                        ...
                                        if(replyChat.attachment_name == "")
                                        {
                                            let otherReplyCell : ReplyViewCell = tableView.dequeueReusableCell(withIdentifier: "replyotherCell", for: indexPath) as! ReplyViewCell
                                            ...
                                            {
                                                otherReplyCell.otherUser.text = replyChat.username!
//                                                otherReplyCell.otherUser.attributedText = friendNameWithDegree(String(format:"%@",replyChat.username!), degreeofUser(friendObj))
                                            }
                                            if decode(replyChat.message!) != nil
                                            {
                                                let msgReplyText : String = decode(replyChat.message!)!
                                            ...
                                            otherReplyCell.userName.text = chatuser.username!
//                                             otherReplyCell.userName.attributedText = friendNameWithDegree(String(format:"%@",chatuser.username!), degreeofUser(friendObj))
...
//                                            if(is_like == false)
                                        ...
                                        else
                                        {
                                            if(replyChat.attachment_name?.lowercased() == MediaType.Image.rawValue)
                                            {
                                                let otherReplyImageCell : ReplyChatOtherCell = tableView.dequeueReusableCell(withIdentifier: "replyOtherImage", for: indexPath) as! ReplyChatOtherCell
                                                // let likeCount : Int = chatuser.count!
                                                //let is_like : Int = Int(chatuser.is_like!)!
                                                let date = dateFormaatr.date(from: chatuser.created_at!)
                                                otherReplyImageCell.userName.text = chatuser.username!
//                                                otherReplyImageCell.userName.attributedText = friendNameWithDegree(String(format:"%@",chatuser.username!), degreeofUser(friendObj))
                                                if(chatuser.user_id == userID)
                                                {
                                                ...
                                                {
                                                    otherReplyImageCell.otherUser.text = replyChat.username!
//                                                    otherReplyImageCell.otherUser.attributedText = friendNameWithDegree(String(format:"%@",replyChat.username!), degreeofUser(friendObj))
                                                }
                                                if decode(chatuser.message!) != nil
                                                {
                                                    let msgText : String = decode(chatuser.message!)!
                                            ...
                                            if(replyChat.attachment_name?.lowercased() == MediaType.Video.rawValue)
                                            {
                                                let otherReplyVideoCell : OtherVideoReplyCell = tableView.dequeueReusableCell(withIdentifier: "othervideoReply", for: indexPath) as! OtherVideoReplyCell
                                                
                                                let is_likee : Int = Int(chatuser.is_like!)!
                                                
                                                let date = dateFormaatr.date(from: chatuser.created_at!)
                                                otherReplyVideoCell.userName.text = chatuser.username!
//                                                 otherReplyVideoCell.userName.attributedText = friendNameWithDegree(String(format:"%@",chatuser.username!), degreeofUser(friendObj))
                                                if(chatuser.user_id == userID)
                                                {
                                                ...
                                                {
                                                    otherReplyVideoCell.otherUser.text = replyChat.username!
//                                                    otherReplyVideoCell.otherUser.attributedText = friendNameWithDegree(String(format:"%@",replyChat.username!), degreeofUser(friendObj))
                                                }
                                                if decode(chatuser.message!) != nil
                                                {
...
//                                                    otherReplyVideoCell.playerView?.playerLayer.videoGravity = AVLayerVideoGravity.resizeAspectFill
                                ...
                                {
                                    let otherCell : ChatByOtherCell = tableView.dequeueReusableCell(withIdentifier: "chatbyotherCell", for: indexPath) as! ChatByOtherCell
                                    let date = dateFormaatr.date(from: chatuser.created_at!)
                                    otherCell.chatTime.text = String(format:"%@",timeFormattr.string(from: date!))
//                                    otherCell.userName.attributedText = friendNameWithDegree(String(format:"%@",chatuser.username!), degreeofUser(friendObj))
                                    otherCell.userName.text = chatuser.username!
                                    let tapGesture = UITapGestureRecognizer(target: self, action: #selector(PLocationChat.showProfile(_:)))
                                    otherCell.userImg.isUserInteractionEnabled = true
                                    otherCell.userImg.addGestureRecognizer(tapGesture)
                                    otherCell.userImg.tag = Int(friendObj.id)!
                                    
//                                    otherCell.userStatus.image = kAppDelegate.setUserStatus(status: friendObj.status)
                                    
//                                    setUserImage(imgView:otherCell.userImg,urlStr: friendObj.profile_pic_thumb)
                                    ...
                                    let widthChat : CGFloat = self.widthForLabel(text: chatuser.message!, font: otherCell.userChatText.font, height: 15)
                                    ...
                                    if(widthChat+120 > (UIScreen.main.bounds.size.width))
                                    {
                                    ...
                                    {
                                        if(widthChat < widthUsername)
                                        {
                                            otherCell.trailingConstraint.constant = (UIScreen.main.bounds.size.width ) - (widthUsername+160)
                                            
                                            
                                        }
                                        else
                                        {
                                            otherCell.trailingConstraint.constant = (UIScreen.main.bounds.size.width ) - (widthChat+140)
                            ...
                            else
                            {
                                if(chatuser.attachment_name?.lowercased() == MediaType.Image.rawValue)
                                {
                                    let otherImageCell : ChatImageByOtherCell = tableView.dequeueReusableCell(withIdentifier: "otherImageCell", for: indexPath) as! ChatImageByOtherCell
                                    
                                    let date = dateFormaatr.date(from: chatuser.created_at!)
                                    otherImageCell.lblUserName.text = chatuser.username!
//                                    otherImageCell.lblUserName.attributedText = friendNameWithDegree(String(format:"%@",chatuser.username!), degreeofUser(friendObj))
                                ...
                                if(chatuser.attachment_name?.lowercased() == MediaType.Video.rawValue)
                                {
                                    let otherVideoCell : OtherChatVideoCell = tableView.dequeueReusableCell(withIdentifier: "otherChatVideo", for: indexPath) as! OtherChatVideoCell
                                    
//                                    let is_likee : Int = Int(chatuser.is_like!)!
                                    
                                    let date = dateFormaatr.date(from: chatuser.created_at!)
                                    otherVideoCell.otherUerName.text = chatuser.username!
//                                    otherVideoCell.otherUerName.attributedText = friendNameWithDegree(String(format:"%@",chatuser.username!), degreeofUser(friendObj))
...
//                imgView.image = #imageLiteral(resourceName: "avatar")
    ...
    /*
    func degreeofUser(_ users:PPeople) -> String{
        if users.is_friend == Connection.Accept.rawValue {
            return "1st"
        }else if users.common_friend_name.isEmpty {
            return "3rd+"
        }else{
            return "2nd"
        }
    ...
    func friendNameWithDegree(_ name: String,_ degree: String) -> NSMutableAttributedString {
        
        let wholeStr = "\(name) .\(degree)"
        let boldUsername = NSMutableAttributedString(string: wholeStr)
        boldUsername.addAttribute(NSForegroundColorAttributeName, value: UIColor.black, range: (wholeStr as NSString).range(of: name))
        boldUsername.addAttribute(NSFontAttributeName, value: UIFont.boldSystemFont(ofSize: 14.0), range: (wholeStr as NSString).range(of: name))
        
        boldUsername.addAttribute(NSForegroundColorAttributeName, value: UIColor.darkGray, range: (wholeStr as NSString).range(of: ".\(degree)"))
        boldUsername.addAttribute(NSFontAttributeName, value: UIFont.systemFont(ofSize: 14.0), range: (wholeStr as NSString).range(of: ".\(degree)"))
        
//        boldUsername.addAttribute(NSAttributedStringKey.foregroundColor, value: UIColor.black, range: (wholeStr as NSString).range(of: name))
//        boldUsername.addAttribute(NSAttributedStringKey.font, value: UIFont.boldSystemFont(ofSize: 14.0), range: (wholeStr as NSString).range(of: name))
//
//        boldUsername.addAttribute(NSAttributedStringKey.foregroundColor, value: UIColor.darkGray, range: (wholeStr as NSString).range(of: ".2nd"))
//        boldUsername.addAttribute(NSAttributedStringKey.font, value: UIFont.systemFont(ofSize: 14.0), range: (wholeStr as NSString).range(of: ".2nd"))
        
        
        //        var attributes : [NSAttributedStringKey : Any] = [NSAttributedStringKey.font : UIFont.boldSystemFont(ofSize: 14.0), NSAttributedStringKey.foregroundColor : UIColor.black]
        //
        //
        //        let attStringSaySomething = NSAttributedString(string: name, attributes: attributes)
    ...
    */
    ...
    @objc func bttnPlayPauseVideoSender(sender: UIButton)
    {
        ...
        do
        {
            ...
            let cell : MyChatVideoCell = chatTablView.cellForRow(at: indexPath) as! MyChatVideoCell
    ...
    @objc func bttnmyReplyVideo(sender : UIButton)
    {
        ...
        do
        {
            ...
            let indexPath : IndexPath = IndexPath(row: tagVal, section: 0)
            let cell : MyVideoReplyCell = chatTablView.cellForRow(at: indexPath) as! MyVideoReplyCell
            let chatObj : ChatUser = self.msgArry[tagVal]
    ...
    @objc func bttnPlayPauseOtherVideoSender(sender: UIButton)
    {
        ...
        do
        {
            ...
            let indexPath : IndexPath = IndexPath(row: tagVal, section: 0)
            let cell : OtherChatVideoCell = chatTablView.cellForRow(at: indexPath) as! OtherChatVideoCell
            let chatObj : ChatUser = self.msgArry[tagVal]
            let playerController = AVPlayerViewController()
            playerController.delegate = self
            playerController.showsPlaybackControls = true
        ...
        if(indexPath.row < self.msgArry.count)
        {
            ...
            if(chatObj.attachment_name == "")
            {
                if(parentID > 0 && chatObj.reply != nil)
                {
                    ...
                    {
                        self.showToastAlert(message: "There is no image.")
                    }
            ...
            {
                if(chatObj.attachment_name?.lowercased() == MediaType.Image.rawValue)
                {
...
extension PLocationChat : AVPlayerViewControllerDelegate
{
...
extension PLocationChat
{
    @IBAction func bttnSendReport(sender : UIButton)
    {
        if(selectedReportOpt == "")
        {
            self.showToastAlert(message: "Please select reason first.")
        }
        else
        {
            txtreportReason.resignFirstResponder()
    ...
    func setLikeCountandImage(indexPath : IndexPath , is_like : Int , likeCount : Int)
    {
        ...
        if(cell is ChatByOtherCell)
        {
            let chatByOtherCell : ChatByOtherCell = cell as! ChatByOtherCell
        ...
        else if(cell is ReplyViewCell)
        {
            let replyViewCell : ReplyViewCell = cell as! ReplyViewCell
        ...
        else if(cell is ReplyChatOtherCell)
        {
            let replyChatOtherCell : ReplyChatOtherCell = cell as! ReplyChatOtherCell
        ...
        else if(cell is OtherVideoReplyCell)
        {
            let otherVideoReplyCell : OtherVideoReplyCell = cell as! OtherVideoReplyCell
        ...
        else if(cell is ChatImageByOtherCell)
        {
            let chatImageByOtherCell : ChatImageByOtherCell = cell as! ChatImageByOtherCell
        ...
        else if(cell is OtherChatVideoCell)
        {
            
            let otherChatVideoCell : OtherChatVideoCell = cell as! OtherChatVideoCell
    ...
    func displayShareController(msg : String , attachment : Any)
    {
        var shareItem = [AnyObject]()
        if(msg == "imageShare" )
        {
            shareItem.append(attachment as AnyObject) //[attachment as Data as AnyObject]
        }
        else if(msg == MediaType.Video.rawValue)
        {
            shareItem.append(attachment as AnyObject)
        }
        else
        {
            shareItem =  [msg as AnyObject] //= //[msg as String as AnyObject]
        }
...
//    }
    ...
    @IBAction func bttnmsgReplyClicked(sender : UIButton)
    {
        ...
        if(selectedIndex < msgArry.count && self.selectedIndex > 0)
        {
            ...
            else if(chatObj.attachment_name?.lowercased() == MediaType.Video.rawValue)
            {
                lblMsgOriginalUser.isHidden = true
                lblMsgOriginalUser.isHidden = true
                replyMediaView.isHidden = false
                lblMediaType.text = MediaType.Video.rawValue
...
//            else
//            {
//                if(chatObj.attachment_name?.lowercased() == MediaType.Video.rawValue)
//                {
//
//                }
//                else
...
//    }
    @IBAction func bttnmsgShareClicked(sender : UIButton)
    {
        ...
        if(selectedIndex < msgArry.count && self.selectedIndex > 0)
        {
            ...
            else if(chatObj.attachment_name?.lowercased() == MediaType.Video.rawValue)
            {
                ...
                if(videoName != "")
                {
                    do
                    {
                        ...
                        let urlData = NSData(contentsOf: NSURL(string:String(format:"%@%@",videoBaseURL,chatObj.video!))! as URL)
                        ...
                        if ((urlData) != nil){
                            ...
                            displayShareController(msg: "imageShare", attachment: videoLink)
            ...
            {
                let data = Data.init()
                displayShareController(msg: chatObj.message!, attachment: data)
            }
            
            selectedIndex = -1
        }
        //Facebbok and Twitter will share image when both Apps installed on device.
    }
    ...
    @IBAction func bttnmsgCopyClicked(sender : UIButton)
    {
        ...
        if(selectedIndex < msgArry.count && self.selectedIndex > 0)
        {
            ...
            self.showToastAlert(message:"Message copied")
        ...
        if photos == .notDetermined {
            PHPhotoLibrary.requestAuthorization({status in
                if status == .authorized{
                        ...
                        if(chatObj.attachment_name?.lowercased() == MediaType.Image.rawValue)
                        {
                            ...
                            DispatchQueue.global(qos: .background).async {
                                            ...
                                            do {
                                                PHPhotoLibrary.shared().performChanges({
                                                    ...
                                                    if completed {
                                                        ...
                                                        self.showToastAlert(message: "Photo saved.")
                                                    }
                                                }
                                            }
                                ...
                                /*    SDWebImageDownloader.shared().downloadImage(with: imgURL, options: SDWebImageDownloaderOptions.ignoreCachedResponse, progress: { (receivedSize :Int, ExpectedSize :Int,  url : URL?) in
                                 ...
                                 if completed {
                                 print("Image is saved!")
                                 self.showToastAlert(message: "Image downloaded.")
                                 }
                                 }
                                 }
                                    ...
                                    DispatchQueue.main.async {
                                        ...
                                        PHPhotoLibrary.shared().performChanges({
                                            ...
                                            if completed {
                                                ...
                                                self.showToastAlert(message: "Video saved.")
                                            }
                                        }
                                    }
                                }
                    ...
                    self.showToastAlert(message: "You are not allowed to access your Media Library.")
                }
            })
        }
        else
    ...
    @IBAction func bttnmsgDeleteClicked(sender : UIButton)
    {
        ...
        alertController.addAction(UIAlertAction.init(title: "Yes", style: .default, handler: { (action) in
            ...
            {
                let chatObj : ChatUser = self.msgArry[self.selectedIndex]
                self.showToastAlert(message: "Message deleted")
    ...
    {
        imgViewContainer.removeFromSuperview()
        addAttachment(attachmentName: MediaType.Image.rawValue)
    }
...
extension PLocationChat : UINavigationControllerDelegate , UIImagePickerControllerDelegate
{
        ...
        exportSession!.exportAsynchronously{() -> Void in
            ...
            case .completed:
                ...
                DispatchQueue.main.async {
                    ...
                    self.addAttachment(attachmentName: MediaType.Video.rawValue)
                }
